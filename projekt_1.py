...
def analyze_text(text):
    # Rozdělení textu na slova pomocí regex
    ...
    for word in words:
        ...
        if word.isdigit():
            stats['numeric'] += 1
            stats['sum_numeric'] += int(word)
            continue

        # Zpracování slov
        if len(word) > 1:  # Ignorujeme jednopísmenná slova
            if word.isalpha():  # Pouze slova obsahující písmena
                if word.isupper():
                    # Uppercase slova - musí být celé velkými písmeny
                    stats['uppercase'] += 1
                elif word[0].isupper() and word[1:].islower():
                    # Titlecase slova - první velké, zbytek malé
                    stats['titlecase'] += 1
                elif word.islower():
                    # Lowercase slova
                    stats['lowercase'] += 1
